...
extension UIImage {
    ...
    private static func symbol(named name: String, scale: UIImage.SymbolScale) -> UIImage {
        return UIImage(systemName: name, withConfiguration: UIImage.SymbolConfiguration(scale: scale)) ?? UIImage()
    }
}
